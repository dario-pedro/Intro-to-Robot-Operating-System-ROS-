...
# Copyright (C) 2016  Ricardo Mendonca
#
...
class IRSeekerv2(BaseDigitalSensor):
    """Object for HiTechnic IRSeeker sensors.
       Coded to HiTechnic's specs for the sensor but not tested.
       Please report whether this worked for you or not!
    """
    I2C_ADDRESS = BaseDigitalSensor.I2C_ADDRESS.copy()
    I2C_ADDRESS.update({
        'dspmode': (0x41, 'B'),
...
class Magnetic(BaseAnalogSensor):
    """
    Object for HiTechnic Magnetic sensors.
    """

    def __init__(self, brick, port):
        super(Magnetic, self).__init__(brick, port)
        self.set_input_mode(Type.ANGLE, Mode.RAW)
        self.offset = 0


    def get_magnetic_value(self):
    ...
    def calibrate(self):
        self.set_zero( self.get_magnetic_value() )

    get_sample = get_magnetic_value
...
class Gyro(BaseAnalogSensor):
    ...
    #This class is for the hitechnic gyro sensor. When the gyro is not
    #moving there will be a constant offset that will change with
    #temperature and other ambient factors. The calibrate() function
    #takes the current value and uses it to offset subsequent ones.
